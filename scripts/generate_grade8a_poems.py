#!/usr/bin/env python3
"""为八上 18 首新增诗词写入articles分文件、创建典故注释文件并导入数据库。

Usage:
  python3 scripts/generate_grade8a_poems.py --dry-run      # 验证数据
  python3 scripts/generate_grade8a_poems.py --apply         # 写入并导入数据库
  python3 scripts/generate_grade8a_poems.py --articles-only # 仅写入+导入正文
  python3 scripts/generate_grade8a_poems.py --glossaries-only # 仅创建+导入典故注释
"""
...
def load_poems():
    path = os.path.join(DATA_DIR, "poems_grade8a.json")
    with open(path, "r", encoding="utf-8") as f:
        return json.load(f)

def load_glossaries():
    path = os.path.join(DATA_DIR, "glossaries_grade8a.json")
    with open(path, "r", encoding="utf-8") as f:
        return json.load(f)
...
def write_articles(poems):
    """将 18 首新诗词追加到 articles_grade8a.json。"""
    path = os.path.join(ARTICLES_DIR, "articles_grade8a.json")
    ...
    if not new_poems:
        print("⚠️  All poems already exist in articles_grade8a.json, skipping.")
        return
    ...
    print(f"✅ Added {len(new_poems)} poems to articles_grade8a.json ({total_sentences} sentences, {total_kw} keyWords)")
    print(f"   Total articles in grade8a: {len(existing)}")

def create_glossary_files(glossaries):
    """创建 18 个典故注释 JSON 文件并校验。"""
    created = []
...
def import_articles():
    ...
    grade8a_count = len([a for a in all_articles if a.get("textbook") == "grade8a"])
    print(f"   文件数: {len(files)}, 总篇数: {len(all_articles)}, 八上: {grade8a_count}")
...
def main():
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "--apply":
            ...
            print("\n🎉 Done! 18 grade8a poems backfilled.")
            return
    ...
    print_summary(poems)
    print(f"\n💡 下一步:")
    print(f"   python3 scripts/generate_grade8a_poems.py --dry-run   # 详细检查")
    print(f"   python3 scripts/generate_grade8a_poems.py --apply      # 写入并导入数据库")
