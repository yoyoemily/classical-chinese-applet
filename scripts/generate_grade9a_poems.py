#!/usr/bin/env python3
"""为九上 11 首新增诗词写入articles分文件、创建典故注释文件并导入数据库。

Usage:
  python3 scripts/generate_grade9a_poems.py --dry-run      # 验证数据
  python3 scripts/generate_grade9a_poems.py --apply         # 写入并导入数据库
  python3 scripts/generate_grade9a_poems.py --articles-only # 仅写入+导入正文
  python3 scripts/generate_grade9a_poems.py --glossaries-only # 仅创建+导入典故注释
"""
...
def load_poems():
    path = os.path.join(DATA_DIR, "poems_grade9a.json")
    with open(path, "r", encoding="utf-8") as f:
        return json.load(f)

def load_glossaries():
    path = os.path.join(DATA_DIR, "glossaries_grade9a.json")
    with open(path, "r", encoding="utf-8") as f:
        return json.load(f)
...
def write_articles(poems):
    """将 11 首新诗词追加到 articles_grade9a.json。"""
    path = os.path.join(ARTICLES_DIR, "articles_grade9a.json")
    ...
    if not new_poems:
        print("⚠️  All poems already exist in articles_grade9a.json, skipping.")
        return
    ...
    print(f"✅ Added {len(new_poems)} poems to articles_grade9a.json ({total_sentences} sentences, {total_kw} keyWords)")
    print(f"   Total articles in grade9a: {len(existing)}")
...
def import_articles():
    ...
    grade9a_count = len([a for a in all_articles if a.get("textbook") == "grade9a"])
    print(f"   文件数: {len(files)}, 总篇数: {len(all_articles)}, 九上: {grade9a_count}")
...
def main():
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "--apply":
            ...
            print("\n🎉 Done! 11 grade9a poems backfilled.")
            return
    ...
    print_summary(poems)
    print(f"\n💡 下一步:")
    print(f"   python3 scripts/generate_grade9a_poems.py --dry-run   # 详细检查")
    print(f"   python3 scripts/generate_grade9a_poems.py --apply      # 写入并导入数据库")
