#!/usr/bin/env python3
"""为高二下 4 首新增诗词写入articles分文件、创建典故注释文件并导入数据库。

Usage:
  python3 scripts/generate_grade12a_poems.py --dry-run      # 验证数据
  python3 scripts/generate_grade12a_poems.py --apply         # 写入并导入数据库
  python3 scripts/generate_grade12a_poems.py --articles-only # 仅写入+导入正文
  python3 scripts/generate_grade12a_poems.py --glossaries-only # 仅创建+导入典故注释
"""
...
def load_poems():
    path = os.path.join(DATA_DIR, "poems_grade12a.json")
    with open(path, "r", encoding="utf-8") as f:
        return json.load(f)

def load_glossaries():
    path = os.path.join(DATA_DIR, "glossaries_grade12a.json")
    with open(path, "r", encoding="utf-8") as f:
        return json.load(f)
...
def write_articles(poems, dry_run=True):
    """将新诗词写入 articles_grade12a.json。"""
    target_file = os.path.join(ARTICLES_DIR, "articles_grade12a.json")
    ...
    if not new_poems:
        print("⚠️  All poems already exist in articles_grade12a.json")
        return

    print(f"📝 Adding {len(new_poems)} new poems to articles_grade12a.json...")
    for p in new_poems:
        article = {
            "id": p["id"],
            "title": p["title"],
...
def main():
    ...
    if not (dry_run or apply_mode or articles_only or glossaries_only):
        print("Usage: python3 generate_grade12a_poems.py [--dry-run|--apply|--articles-only|--glossaries-only]")
        sys.exit(1)
