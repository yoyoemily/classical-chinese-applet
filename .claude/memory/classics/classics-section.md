...
| 28 | 唐诗三百首 | 集 | 选集型 | chunked | author | 293 首 ✅ |
...
| `POST /api/admin/import/classic/{classicId}/glossary` | 典故注释独立导入——幂等先删后插，仅写 `classic_glossary` 表，不动正文/译文（与正文解耦的两步工作流用） |
...
`background` 字段（可选）：每条 entry 可填创作背景文本，后端经典导入自动写入 `classic_chapter.background`，前端经典阅读器标题区有背景时显示下划线，点击弹出背景弹窗。唐诗三百首 293 首已全部补充 background。

### 2. 编写典故注释（glossary）

> 这是最容易被遗漏的一步——**正文和译文跑通了 ≠ 做完了**。

**推荐使用独立 glossary.json 文件**。典故注释与正文完全解耦，支持先导入正文、后补注释的两步工作流。
...
[
  {
    ...
    "paragraphs": [{
      "sortOrder": 0,
      "glossary": [{ "word": "兵", "explanation": "此处指战争、军事..." }]
    }]
  }
]
```
...
[
  {
    ...
    "entries": [{
      ...
      "paragraphs": [{
        "sortOrder": 0,
        "glossary": [{ "word": "陈仲举", "explanation": "即陈蕃..." }]
      }]
    }]
  }
]
...
- **章节型匹配**：`(classic_id, chapterTitle, sortOrder)` → 段落
- **选集型匹配**：`(classic_id, chapterTitle, entryTitle, sortOrder)` → 段落
- **幂等**：先删后插，安全重复执行
- **渐进**：只更新 glossary.json 中引用的段落，不影响其他段落
- **导入命令**：`curl -X POST localhost:8080/api/admin/import/classic/{id}/glossary -H "Content-Type: application/json" -d @glossary.json`
- **旧格式兼容**：embed 在 paragraphs JSON 中的 glossary 仍可通过原端点导入。`DataImportService.importClassicGlossary()` 自动检测 entries 字段判断章节型/选集型。

**也可以在正文 JSON 中嵌入 glossary**（旧方式，仍支持），后端 `importClassicBook()` 自动写入 `classic_glossary` 表，前端 `buildSegment()` 自动最长匹配高亮 + 点击弹泡。无需改任何代码。

**核心原则**：凡是能增加阅读乐趣的文化内容都标注，读懂 + 有乐趣，不搞文学分析。

**标注范围**（经典面向文化通识，与选篇略有不同）：

| 标注 ✅ | 不标 ✗ |
|----------|--------|
| 典故、历史人名与事件 | 基础实词虚词释义（归词书管） |
| 古代官职、地名、制度 | 现代读者凭上下文可推断的词 |
| 文化背景词（需解释才能理解） | 常见名词（春年山日）——现代汉语常用 |
| 器物、草木、动物 | 现代汉语极度常见的词（的、了、是、我、你、他…） |
| 名句、哲理表达 | 词书已覆盖的实词/虚词 |
| 引经据典的出处（如引用《诗经》《论语》等） | 文学技法、修辞手法、文章结构——不搞文学分析 |
| 历史事件、文化背景概念 | 学生 100% 确定的词 |
| 与作者生平/时代背景相关的关键信息 | |
| 词书已覆盖但此处有独特文化含义的实词 | |
...
- 每段落的 `glossary` 数组可以为空（表示暂无需要注释的词）
- 每条 definition 以中文句号结尾
- 引用原文使用中文直角引号「」
- 前端自动最长匹配切分高亮，无需 `startIndex`
...
经典典故注释（glossary）已支持与正文 JSON 分离。后端新增 `POST /api/admin/import/classic/{id}/glossary` 端点，通过自然键（标题 + 段落序号）匹配已有段落，幂等写入 `classic_glossary` 表，不动正文/译文。

**全部典故注释已清空。** 当前阶段聚焦经典原文和译文的编撰与导入，典故注释延后到全部经典上线后再统一铺排。glossary.json 文件暂时保留在知识库中（作为独立文件），但暂不导入数据库。
...
|------|----|------|---------|------|------|
| 孙子兵法 | 22 | 章节型 | strip | 13 篇，78 段落 | ✅ 已上线 |
| 老子 | 18 | 章节型 | list | 81 章，6410 字 | ✅ 已上线 |
| 世说新语 | 33 | 选集型 | accordion | 36 门 54 章 | ✅ 已上线 |
| 山海经 | 36 | 卷帙型 | accordion | 18 卷 | ✅ 已上线 |
| 唐诗三百首 | 28 | 选集型 | author | 318 章，293 条创作背景 | ✅ 已上线 |
| 宋词三百首 | 29 | 选集型 | author | 235 首词，33 个目录分组，78 位作者（52 首归入"其他"组），chunked + author | ✅ 已上线 |
| 大学 | 3 | 章节型 | strip | 1 篇，26 段落 | ✅ 已上线 |
| 中庸 | 4 | 章节型 | list | 1 篇，52 段落 | ✅ 已上线 |
| 三字经 | 43 | 章节型 | strip | 8 章，25 段落 | ✅ 已上线 |
| 千字文 | 44 | 章节型 | strip | 11 章，23 段落 | ✅ 已上线 |
| 孝经 | 38 | 章节型 | strip | 18 章，18 段落 | ✅ 已上线 |
| 楚辞 | 27 | 章节型 | strip | 8 篇，32 段落 | ✅ 已上线 |
| 鬼谷子 | 24 | 章节型 | strip | 12 篇，64 段落 | ✅ 已上线 |
| 菜根谭 | 47 | 章节型 | list | 60 则，60 段落 | ✅ 已上线 |
| 浮生六记 | 55 | 章节型 | list | 4 章，21 段落 | ✅ 已上线 |
| 周易 | 5 | 章节型 | list | 64 卦，253 段落 | ✅ 已上线 |
| 荀子 | 17 | 章节型 | list | 32 篇，60 段落 | ✅ 已上线 |
| 韩非子 | 20 | 章节型 | list | 16 章，16 段落 | ✅ 已上线 |
| 墨子 | 21 | 章节型 | list | 8 章，8 段落 | ✅ 已上线 |
| 列子 | 37 | 章节型 | list | 8 篇，26 段落 | ✅ 已上线 |
| 颜氏家训 | 40 | 章节型 | list | 20 篇，21 段落 | ✅ 已上线 |
| 围炉夜话 | 58 | 章节型 | list | 184 则，184 段落 | ✅ 已上线 |
| 晏子春秋 | 46 | 章节型 | list | 8 篇，23 段落 | ✅ 已上线 |
...
> **当前策略**：聚焦经典原文和译文的编撰与导入，典故注释（glossary）延后到全部经典上线后再统一铺排。所有已上线经典的 glossary 已清空。

25 部已上线经典中，17 部基本完整，8 部有不同程度的内容缺失：

**✅ 基本完整（17 部）**：宋词三百首、老子、周易、中庸、孙子兵法、唐诗三百首、三字经、孝经、千字文、围炉夜话、大学、菜根谭、鬼谷子、楚辞（32段，7,453字）、列子（26段，7,163字）、颜氏家训（21段，5,304字）、晏子春秋（23段，5,189字）

**⚠️ 内容偏薄但目录全（0 部）**：无

**❌ 严重残缺（8 部）**：荀子（~93%缺段落）、韩非子（~98%缺 39 章+内容）、墨子（~98%缺 45 章+内容）、世说新语（~97%缺条目）、山海经（~60%缺段落）、西厢记（~97%缺）、浮生六记（~80%缺）、陶渊明集（~80%缺，仅选 15 首）

**❌ 空壳（0 部）**：无

**修复优先级**：韩非子/墨子（名篇优先补全）→ 世说新语（1100+条目分批导入）→ 荀子（目录全，补段落即可，体量小）

---

## 生僻字拼音旁注

经典阅读器和选篇阅读器均已支持生僻字拼音旁注。
...
| 后端 service | `service/DataImportService.java` | `importClassicBook()` 自动检测类型（entries vs paragraphs）→ 二级/一级导入；`importClassicGlossary()` 典故注释独立导入（章节型/选集型自动检测） |
