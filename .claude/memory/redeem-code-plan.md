...
# 分享门禁 → 公众号学习码兑换 改造计划（已废弃）

> **2026-07-29 废弃**：门禁已简化为「累计打卡满N天 + 非契约会员 → 弹窗引导跳转我的页面」。不再依赖微信公众号、学习码、签契。本文件仅保留历史参考。

## Context

**旧方案**（已废弃）：门禁改为关注服务号 → 获取学习码 → 输入验证 →（首次）签金石契。
...
CREATE TABLE redeem_code (
  ...
  code        VARCHAR(32)  NOT NULL UNIQUE COMMENT '兑换码，8 位数字',
  user_id     BIGINT       COMMENT '所属小程序用户ID（公众号生成时为NULL，用户输入后认领）',
  mp_open_id  VARCHAR(64)  COMMENT '公众号 OpenID（扫码关注时记录）',
...
| `POST /api/admin/generate-code` | POST（无参） | 管理端生成学习码（8 位数字，不绑定用户，谁输入谁认领） | ✅ |
...
| `service/UserService.java` | 新增 `verifyCode()`、`getMemberStatus()`、`generateCode()`、`generateMpCode()`；`verifyCode()` 支持认领未绑定码（userId=NULL→绑定当前用户）；`signPact()` 增加前置校验；`buildMemberStatus` 返回 `codeStatus`（-1/1/2）；`getUserProfile()` 返回 `codeStatus` |
| `controller/UserController.java` | 新增 `POST /api/user/verify-code`、`GET /api/user/member-status` |
| `controller/ImportController.java` | 新增 `POST /api/admin/generate-code`（无参，不绑定用户） |
| `data/schema.sql` | `user` 表加 `last_active_at` 列；`redeem_code` 表 `user_id` → 可空，新增 `mp_open_id` 列 + 索引 |
...
| 阶段二 | 输入框（8位数字纯digit） + "确认验证" + 错误提示 + 返回关注 | 用户点击"输入学习码" |
...
- 未过期（codeStatus=-1）：🎉 坚持了N天 + "关注公众号，获取永久免费学习码"
- 已过期（codeStatus=2）：⏰ 好久不见！你的学习码已过期 + "请重新关注公众号，获取新的学习码"

**学习码输入**：`type="digit"` 纯数字键盘，`onInputCode` 过滤 `\D` 截断 8 位
...
```
从没绑过码：打卡满N天 → 弹窗阶段一（🎉 欢迎）→ 阶段二（输入8位数字码）→ 阶段三（签契）→ 关闭弹窗，用户自行点击开始学习
...
| 文件 | 内容 |
|------|------|
| `config/WechatMpProperties.java` | 服务号配置属性类（`wechat.mp.*`） |
| `config/WechatMpConfig.java` | `WxMpService` Bean 配置 |
| `service/WechatMpService.java` | 核心逻辑：subscribe 事件 → `generateMpCode()` → 客服消息回复学习码 |
| `controller/WechatMpController.java` | `GET/POST /api/wechat/mp/portal` — 微信服务器验证 + 接收消息/事件 |

### 4.2 修改文件

| 文件 | 改动 |
|------|------|
| `pom.xml` | 新增 `weixin-java-mp` 4.7.0 依赖 |
| `config/WebMvcConfig.java` | `/api/wechat/mp/**` 加入放行路径（不走登录拦截器） |
| `entity/RedeemCode.java` | `user_id` 改为可空（Long），新增 `mp_open_id` 字段 |
| `service/UserService.java` | `verifyCode()` 支持认领未绑定码（userId=NULL→绑定当前用户）；新增 `generateMpCode(mpOpenId)` 公众号生成码方法 |
| `data/schema.sql` | `redeem_code` 表：`user_id` → 可空，新增 `mp_open_id` 列 + 索引 |
| `application-dev.yml` | 新增 `wechat.mp` 配置节（appId/secret/token/aesKey） |
| `application-prd.yml` | 同上 |

### 4.3 流程

```
用户扫码关注服务号
  → 微信推送 subscribe 事件到 POST /api/wechat/mp/portal
  → WechatMpService 接收 XML，提取 FromUser (MP OpenID)
  → UserService.generateMpCode(mpOpenId)：生成 8 位数字码（userId=NULL, mpOpenId=FromUser）
  → 客服消息回复用户："你的学习码：XXXXXX，请在文言雀小程序中输入"
  → 用户在小程序门禁页输入码
  → UserService.verifyCode() 找到码，userId=NULL → 认领给当前小程序用户
  → 验证通过
```

### 4.4 上线步骤

1. **数据库迁移**：在 MySQL 执行 `ALTER TABLE redeem_code MODIFY user_id BIGINT NULL` 和 `ALTER TABLE redeem_code ADD COLUMN mp_open_id VARCHAR(64) COMMENT '公众号 OpenID'` 并 `CREATE INDEX idx_redeem_mp_open_id ON redeem_code(mp_open_id)`
2. **服务号配置**：在 MP 后台 → 设置与开发 → 基本配置 → 服务器配置：
   - URL：`https://wyq.yinqueai.com/api/wechat/mp/portal`
   - Token：`wyqMp2026Token`（与 yml 一致）
   - 编码方式：明文模式（或安全模式，需配 aesKey）
3. **环境变量**：服务器上设置 `WECHAT_MP_APPID`（服务号 AppID）、`WECHAT_MP_SECRET`（服务号 AppSecret）

## 五、待完成

| 项目 | 说明 |
|------|------|
| 服务号审批通过后上线 | 配置 URL + Token，设置环境变量 `WECHAT_MP_APPID` / `WECHAT_MP_SECRET` |
| 服务号二维码图片 | `/assets/qrcode-mp.jpg` 已替换为真实二维码 |

**管理端测试生成码**（服务号审批前可用）：
```bash
curl -X POST http://localhost:8080/api/admin/generate-code
# → {"code": 0, "data": {"code": "382914"}}
```
