import { fetchWordBooks, fetchProgress, fetchTodayTask } from '../../api/index';
...
Page<IIndexData, WechatMiniprogram.Page.CustomOption>({
  ...
  async loadData(): Promise<void> {
    ...
    try {
      ...
      // 并行拉取词书列表、进度、今日任务
      const [books, progress, task] = await Promise.all([
        fetchWordBooks(),
  ...
  /** 点击生词本入口 → 跳转生词本页（默认「全部」tab） */
  onTapVocabulary(): void {
    wx.navigateTo({ url: '/pages/vocabulary/index?tab=all' });
  },
